...
def plotData():
    ...
    cmass = TCanvas("fig_example1", "Z1mass/Z2mass",
                    10, 10, 500, 500)    
    ...
    bntuple = Ntuple(['../public/ntuple_ZZ4L.root',
                      '../public/ntuple_HZZ4L.root'],
                      'Analysis')
    btotal  = 0.0
    total   = 0
    for ii, event in enumerate(bntuple):
        btotal += event.weight
        total  += 1
        hb.Fill(event.Z1mass, event.Z2mass, event.weight)
        if total % 100 == 0:
            cmass.cd()
            hb.Draw('box')
            cmass.Update()
...
def main():
    ...
    # Write out best cut
    # -------------------------------------------------------------
    bestcuts = writeResults('results_box.txt',
                                ntuple, variables,
                                bestrow, bestZ,
                                totals)
