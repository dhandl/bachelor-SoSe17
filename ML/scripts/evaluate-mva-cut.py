...
def save_events(name, input_name, events, err):
	...
	directory = "/lustre/boerner/diagonal/exp_events/"  + name + "/"
...
def main():
	...
	sample = {
		opts.input: DIRECTORY + "/" + opts.input + "/*.root/" + opts.input + "_Nom"
	}

	tree = load_tree(sample, PRESELECTION)

	variables = load_var_list("config/{cfg}.py".format(cfg=opts.variables))
	reader, var_store = create_reader(variables, opts.name + "_" + opts.mva_name)
