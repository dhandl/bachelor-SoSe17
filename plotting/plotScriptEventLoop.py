...
wwwDir = "/afs/cern.ch/user/d/dhandl/www/Run2/SUSY/Stop1l/presel_mt130_met230/"
if not os.path.exists(wwwDir):
  os.makedirs(wwwDir)

fileName = "presel_mt130_met230"
...
cut = "(stxe_trigger) && (n_jet>=4) && (jet_pt[0]>50000) && (jet_pt[1]>25000) && (jet_pt[2]>25000) && (jet_pt[3]>25000) && (mt>130e3) && (met>230e3) && (n_bjet>0) && (dphi_jet0_ptmiss > 0.4) && (dphi_jet1_ptmiss > 0.4) && !((mT2tauLooseTau_GeV > -0.5) && (mT2tauLooseTau_GeV < 80)) &&" + TIGHTE
...
allSignal = [
  {"name":"stop_bWN_350_200", "legendName":"m(#tilde{t},#tilde{#chi}_{1}^{0})=(350,200)", "target":bkgDir+"stop_bWN_350_200/*", "color": ROOT.TColor.GetColor(ROOT.kBlue+2), "chain_name":"stop_bWN_350_200_Nom"},
]
...
m3 = {'name':'myM3', "fileName":fileName+"_M3"+normString+logString, 'varFunc':funcs.mThree, 'Xtitle':'m_{3}', 'Ytitle':'Events', 'binning':[25,0,1000], "binningIsExplicit":False}
lp = {'name':'myLp', "fileName":fileName+"_Lp"+normString+logString, 'varFunc':funcs.getLp, 'Xtitle':'L_{P}', 'Ytitle':'Events', 'binning':[30,-1.5,1.5], "binningIsExplicit":False}

allVariables.append(m3)
...
for sample in allBkg+allSignal:
  ...
  sample["chain"].Draw(">>eList",cutString) #Get the event list 'eList' which has all the events satisfying the cut
  ...
  for i in range(number_events): #Loop over those events
    ...
    weight = funcs.getWeight(sample['chain'])
    #weight = 1.
    ...
    for var in allVariables:
      ...
      varValue = funcs.getVarValue(sample["chain"], var['varString']) if var.has_key('varString') else var['varFunc'](sample["chain"])
      #print varValue, weight
