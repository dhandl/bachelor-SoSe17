...
def get_sig_hist(sig_mlp_dist, bkg_mlp_dist, total_sig, total_bkg, lumi):
  ...
  for bin in xrange(1, sig_mlp_dist.GetNbinsX()):
    ...
    if cut_bkg > 0.:
      rel_err_bkg = cut_bkg_err / cut_bkg
    else: 
      rel_err_bkg = 0.
    tot_err_bkg = math.sqrt(rel_err_bkg*rel_err_bkg + 0.25*0.25)
    ...
    if eff_sig == 0 or eff_bkg == 0:
      ...
      print lumi*1000*eff_sig * total_sig, lumi*1000*cut_sig_err, lumi*1000*eff_bkg * total_bkg, lumi*1000*cut_bkg_err, rel_err_bkg, tot_err_bkg
      Z = get_sig_RooStats(eff_sig * total_sig, eff_bkg * total_bkg, lumi*1000, tot_err_bkg)
